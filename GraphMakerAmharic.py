...
def tokenize(text):
    ...
    tokens_list = [token.strip(punct) for token in re.split(delim_pattern, text)]
    tokens = [token for token in tokens_list if token]
    return tokens   
    


def tokenize_sentences(sentences):
    sents_tokenized = [tokenize(sentence.lower()) for sentence in sentences]
    sentences_tokenized = []
        ...
        try:
            ...
            for sublist_of_sentences in list_of_lists_of_sentences:
                ...
                if graph_number == max_number_of_graphs_per_language:
                    ...
                    break
                #print(f"Graph number is now {graph_number}")
                ...
                #print(f"Writing sublist of sentences {graph_number}:")
                #fileno = f"{output_path}/{fname[:-4]}_{sys.argv[3]}_{graph_number}.txt" 
                #with open(fileno, "w") as f:
                 #   f.write("\n".join(sublist_of_sentences))
                #print(f"Written sublist of sentences {graph_number} ")
                graph_number += 1
