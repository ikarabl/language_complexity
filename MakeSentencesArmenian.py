...
def make_sentences(text):
    ...
    pattern = re.compile("։|\:|՜|\!|\?|…")
    sents_list = [sent.strip() for sent in re.split(pattern, text)]
    sentences = [sent for sent in sents_list if (len(word_tokenize(sent)) > 1) and (len(word_tokenize(sent)) <= 300)]
    return sentences
